 ...
 *
 * Uses 16-bit signed PCM at 16kHz to match the device firmware.
 *
...
const { MicrostreamServer, Protocol, AudioBuffer } = require('microstream-server')

const { MessageType, decode, encodeAudioData, encodeAudioEnd, encodeHeartbeat } = Protocol

// --- Config (matches firmware: 16kHz 16-bit mono) ---
const PORT = 5111
...
function generateSinePcm () {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    const sample = Math.round(Math.sin(2 * Math.PI * TONE_HZ * t) * 32767)
...
server.listen(() => {
  ...
  ws.on('close', () => {
    ...
    console.log('\nDone! Compare the WAV files in server/audio/')
...
function sendAudio (ws) {
  const chunkSize = 512 * 2 // 512 samples * 2 bytes per sample
  let sent = 0
